...
- **The grade registry stays READ ONLY.** `tests/unit/test_registry_is_read_only.py` checks two
  ...
  and (2) every BOUND adapter, in every profile, carries no public attribute starting with a
  write verb. Both were shown red against a planted `set_grade` on the local adapter: the first
  stays green, which is precisely why the second exists. `make portability` and
  `make demo-selftest` fail on the same mutation from two other entry points.
- **A number the engine compares against belongs in the policy block**, not in
  `early_warning.py`, and every row in that block must be READ by something. An inert knob beside
  a live one is worse than an absent one: it reads as a control in an audit and retuning it warns
  nobody. `tests/unit/test_policy.py` holds the shipped `config/settings.yaml` block equal to the
  shipped code defaults, so the two cannot drift, and `validate_policy` refuses an inconsistent
  or INCOMPLETE block at LOAD rather than at first request: a mapping in the block replaces the
  shipped one wholesale, so it must price every row the engine will index.
...
| # | File | What to add | Enforced by |
|---|---|---|---|
| 1 | `src/credit_portfolio_ews/adapters/<family>/<port>.py` | The class. Exactly one constructor shape: `def __init__(self, settings: Settings)`. Cloud SDK imports go INSIDE the method that needs them. | `test_port_parity.py::test_adapter_constructs_with_a_single_settings_argument` |
| 2 | `src/credit_portfolio_ews/config.py` | The `module:Class` target in `DEFAULT_BINDINGS[<port>][<profile>]`. | `test_port_parity.py::test_every_port_binds_every_known_profile` |
