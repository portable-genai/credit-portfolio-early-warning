...
from credit_portfolio_ews.adapters.local.audit import (
    LocalAuditAdapter,
)
from credit_portfolio_ews.adapters.local.tracer import (
...
#: These tests are about the audit chain and the engine, not about tracing, so the container
#: binds the offline no-op rather than a mock nobody would assert on.
_NOOP_TRACER = LocalNoopTracerAdapter(Settings(profile="local"))

#: Three obligors, so the chain has an interior record and a tail worth truncating.
_OBLIGORS = (
    sample_cases.ESCALATING_OBLIGOR,
    sample_cases.ROUTINE_OBLIGOR,
    sample_cases.PII_OBLIGOR,
)

_ANCHOR_ENV = "CREDITEWS_AUDIT_ANCHOR"
...
def test_an_append_after_truncation_cannot_relaunder_the_anchor(tmp_path: Path) -> None:
    ...
    with pytest.raises(AuditChainError):
        build_review_service(build_container(settings)).review(
            sample_cases.ESCALATING_OBLIGOR,
            tenant=sample_cases.TENANT,
            actor=sample_cases.ACTOR,
            as_of=sample_cases.AS_OF,
