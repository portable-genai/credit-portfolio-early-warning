...
def test_the_demo_data_is_obviously_fictional() -> None:
    from credit_portfolio_ews.adapters.local import _fixtures

    obligors = [_fixtures.fixture_for(oid) for oid in _fixtures.OBLIGOR_IDS]
    for fixture in obligors:
        name = fixture.record.name
        assert "FICTIONAL" in name, name + " does not announce itself as fake"
    blob = " ".join(
        [demo.ACTOR, *(fixture.planted_identifier for fixture in obligors)]
        + [term.description for fixture in obligors for term in fixture.terms]
        + [item.snippet for fixture in obligors for item in fixture.news]
    )
    ...
    assert demo.PLANTED_NRIC in blob, "the redaction beat has nothing to look for"


def test_the_demo_walks_three_obligors_from_the_one_shared_estate() -> None:
    """The demo, the local adapters and the eval must not run on estates that merely look alike."""
    from credit_portfolio_ews.adapters.local import _fixtures

    for obligor_id in (demo.ROUTINE_OBLIGOR, demo.ESCALATING_OBLIGOR, demo.PII_OBLIGOR):
        assert obligor_id in _fixtures.OBLIGOR_IDS
...
def test_the_renderer_escapes_content_rather_than_interpolating_it() -> None:
    state = {
        ...
        "region": "r",
        "step_count": 1,
        "totals": {"reviewed": 0, "escalated": 0, "routed": 0, "chain_ok": True},
        "steps": [
            {
                "key": "k",
                "label": "l",
