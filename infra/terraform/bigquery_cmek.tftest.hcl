...
run "every_table_declares_the_datasets_key" {
  ...
  assert {
    condition     = length(google_bigquery_table.obligor_metrics.encryption_configuration) == 1
    error_message = "google_bigquery_table.obligor_metrics must declare the key the dataset stamps on it; an undeclared block reads as a removal at the next plan, which REPLACES the table and destroys every row it holds."
  }

  assert {
    condition     = length(google_bigquery_table.obligor_servicing.encryption_configuration) == 1
    error_message = "google_bigquery_table.obligor_servicing must declare the key the dataset stamps on it; an undeclared block reads as a removal at the next plan, which REPLACES the table and destroys every row it holds."
  }
