...
(``CREDITEWS_REVIEW_ROUTING=off``), the container binds :class:`DisabledReviewRouter` instead of
the profile's class. It satisfies the port and submits nothing, and the container logs the
posture at startup.

**Recording wrapper.** Every caller that hands a result to the router (the review route, the
agent tool, the CLI, each through the review service they build) wraps the bound router in
...
rather than failing an already-assessed, already-audited watchlist review, but it is never
invisible: the caller reports ``failed`` and an empty reference, which nobody can mistake for a
reviewed result.
"""
...
class DisabledReviewRouter:
    ...
    def route(self, result: WatchlistReview, *, maker: str, tenant: str = "") -> str:
        return ""
...
class RecordingReviewRouter:
    ...
    def route(self, result: WatchlistReview, *, maker: str, tenant: str = "") -> str:
        ...
        if not result.assessment.requires_human_review:
            return ""
