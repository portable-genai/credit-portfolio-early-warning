...
and the same per-call temperature (passed through, so a drafting call that leaves it ``None``
sends none), so the two lanes differ in the model and nothing else. The kit client notes the
model that answered for the console's model pill itself.
...
def configured_model() -> str:
    """The model id this lane calls, as the model pill first names it (``LOCAL_MODEL``)."""
    ...
    def generate(self, prompt: str, *, temperature: float | None = None) -> str:
        ...
        try:
            completion = self._client.complete_json(
                messages, temperature=temperature, max_tokens=MAX_OUTPUT_TOKENS
            )
