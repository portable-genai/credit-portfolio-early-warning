...
from .domain.policy import EarlyWarningPolicy, load_policy
from .domain.watchlist_service import WatchlistReviewService
...
if TYPE_CHECKING:  # pragma: no cover - typing only; adapters.controls imports this module
    from .adapters.controls import RecordingReviewRouter

_PROFILE_ENV = "CREDITEWS_PROFILE"
_SETTINGS_ENV = "CREDITEWS_SETTINGS"
...
#: The port whose ACTIVE binding decides what the model pill first says, and the
#: dotted ``Settings`` path holding that model's id. This service binds a generative port for
#: memo drafting and media categorisation, so the pill follows THAT binding. The model-id
#: attribute stays empty on purpose: the managed adapter declares its pinned model as a module
#: constant, and reading it off the binding is what stops a second settings string drifting from
#: the adapter that is actually called.
...
class ProfileChoice:
    ...
    The variable is ``CREDITEWS_PROFILE``, named once in
    :data:`_PROFILE_ENV`.
    ...
    ``os.environ.get("CREDITEWS_PROFILE", "local")``: that fallback reads an
...
def _read_settings_file(path: Path | None = None) -> dict[str, Any]:
    ...
    An EXPLICIT path (argument or ``CREDITEWS_SETTINGS``) that does not
...
def _policy_from(data: Mapping[str, Any]) -> Mapping[str, Any]:
    """Adopt the file's ``policy:`` block, or an empty mapping when it declares none.

    An ABSENT block takes the shipped reference defaults. A block that is PRESENT is honoured as
    written, including a key set to an empty list: an operator who wrote one expressed an intent
    and it is not the same as writing nothing. ``domain/policy.load_policy`` applies that rule and
    validates the result at LOAD, so a ladder that is not monotonic refuses in the deploy rather
    ...
    if block is None:
        return {}
    if not isinstance(block, dict):
        raise ValueError("settings 'policy' must be a mapping")
...
REVIEW_ROUTING_ENV = "CREDITEWS_REVIEW_ROUTING"
...
class Settings:
    ...
    #: The parsed ``policy:`` block: every bank-owned number the early-warning engine compares
    #: against. An empty mapping means no policy was configured, so the shipped reference
    #: defaults in ``domain/policy.py`` apply.
    ...
    def generator_model(self) -> str:
        ...
        calls: the constant the bound managed adapter passes to its call, never a model a flag
        could swap in.
    ...
    def load(cls, path: Path | None = None) -> Settings:
        ...
        settings = cls(
            ...
            credit_memo_url=str(data.get("credit_memo_url") or ""),
            metrics_dataset=str(data.get("metrics_dataset") or ""),
            knowledge_base_url=str(data.get("knowledge_base_url") or ""),
            grade_registry_url=str(data.get("grade_registry_url") or ""),
            adapters=_bindings_from(data),
            policy=_policy_from(data),
...
class Container:
    ...
    @cached_property
    def covenant_terms(self) -> CovenantTermsPort:
        adapter = self._bind("covenant_terms")
        assert isinstance(adapter, CovenantTermsPort)
        return adapter

    @cached_property
    def portfolio_feed(self) -> PortfolioFeedPort:
        adapter = self._bind("portfolio_feed")
        assert isinstance(adapter, PortfolioFeedPort)
        return adapter

    @cached_property
    def adverse_media(self) -> AdverseMediaPort:
        adapter = self._bind("adverse_media")
        assert isinstance(adapter, AdverseMediaPort)
        return adapter

    @cached_property
    def grade_registry(self) -> GradeRegistryPort:
        adapter = self._bind("grade_registry")
        assert isinstance(adapter, GradeRegistryPort)
        return adapter
...
def build_policy(settings: Settings) -> EarlyWarningPolicy:
    """The bank-owned policy for this deployment, validated at LOAD.

    An absent ``policy:`` block takes the shipped reference defaults. A present one is parsed and
    then REFUSED if it is internally inconsistent, so a band ladder that is not monotonic or a
    cap that would let categorised media classify an obligor alone fails the deploy rather than
    the first request.
    """
    return load_policy(settings.policy)


def build_review_service(
    container: Container, *, routing: RecordingReviewRouter | None = None
) -> WatchlistReviewService:
    """Wire the domain service to the container's bound ports. ONE wiring site, not three.

    The API, the CLI and the agent tools all call this, so a port added to the service cannot be
    wired on two surfaces and forgotten on the third.

    ``routing`` is the caller's recording wrapper around the bound review router. A surface that
    returns a review passes one and reports its outcome (``review_routing``): the wrapper turns
    a failed hand-off into ``failed`` and a logged warning rather than a failed request, so the
    domain stays unchanged and the caller still says what happened. Without one, the bound
    router is used directly.
    """
    return WatchlistReviewService(
        ...
        policy=build_policy(container.settings),
    )
