...
``CREDITEWS_PROFILE`` is NO CHOICE rather than a silent ``local``. Every
...
``CREDITEWS_S2S_TOKEN`` is wrong in the most dangerous direction:
...
header and no ``X-Dev-Persona`` then got the seeded persona list and a real grade proposal.
...
from ..domain.errors import ObligorNotFoundError
...
_INSECURE_DEMO_ENV = "CREDITEWS_ALLOW_INSECURE_DEMO"
_CORS_ORIGINS_ENV = "CREDITEWS_CORS_ORIGINS"
_ALLOWED_CALLERS_ENV = "CREDITEWS_S2S_ALLOWED_CALLERS"
_AUDIENCE_ENV = "CREDITEWS_S2S_AUDIENCE"
_API_HOST_ENV = "CREDITEWS_API_HOST"
...
def require_service_caller(request: Request) -> None:
    ...
    stays OPEN when ``CREDITEWS_S2S_TOKEN`` is unset (loopback dev with zero
    secrets), so an UNSET ``CREDITEWS_PROFILE`` must never be allowed to
    ...
    ``CREDITEWS_ALLOW_INSECURE_DEMO=1``.
    """
    if not _request_choice(request).service_auth_configured:
        raise HTTPException(
            ...
            detail=(
                ...
                "CREDITEWS_PROFILE is not set, so no authentication scheme "
                "has been chosen"
            ),
        )
...
_APP_TITLE = "Credit Portfolio Early Warning"
...
#
# Note what is NOT in this expression: CREDITEWS_S2S_TOKEN. A service
...
@app.post("/v1/watchlist-review", response_model=WatchlistReviewResponse, tags=["artifacts"])
def watchlist_review(
    request: WatchlistReviewRequest,
    principal: Annotated[Principal, Depends(get_principal)],
) -> WatchlistReviewResponse:
    """Review one obligor and PROPOSE a watchlist grade. Nothing is applied here, ever.

    The audit actor and the review maker are both the verified principal, never the request
    body. The tenant comes from the resolved principal too, so an obligor under another tenant
    answers 403 and never 404: the two statuses must not be usable to enumerate another bank's
    book.

    ``as_of`` and ``test_period`` are RESOLVED here when the caller left them empty, and echoed
...
def obligors(
    ...
    Scoped to the resolved principal's tenant. There is no write counterpart on this surface or
    on any other: the grade registry port declares read methods only.
    """
    container = _container()
    tenant = principal.tenant or container.settings.tenant
    return [
        ObligorSummary(
            ...
            current_grade=record.current_grade.value,
            currency=record.currency,
            clean_periods=record.clean_periods,
            last_review_on=record.last_review_on.isoformat() if record.last_review_on else "",
        )
        for record in build_review_service(container).obligors(tenant)
    ]
