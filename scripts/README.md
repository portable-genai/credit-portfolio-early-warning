...
| Script | What it does |
|---|---|
| `demo.py` | The scripted arc, driving the REAL services over the `local` profile: bind the stack and show the bank's own policy numbers beside a grade registry that declares no write method, review an obligor whose immaterial arrears must NOT start the past-due clock, review one where the arrears floor and not the composite sets the grade and route it (rule R8), review one whose covenant clause names a guarantor and prove the identifier is masked before the audit write, the model prompt and the wire, show the credit officer's queue, verify and export the audit trail, rewrite a substandard proposal back to a pass and detect it, then swap to the exit profile and watch every seam refuse. Writes the audit-view JSON. |
| `render_calibration_sample.py` | Regenerates the SYNTHETIC calibration sample `eval/datasets/synthetic_calibration.jsonl` from the generative model recorded in the script. No real obligors and no realised outcomes: it exists so the model-risk harness has something to measure the scorecard's structure against while no historical sample exists. `--check` fails the gate when the committed file is stale. |
| `render_evals_doc.py` | Regenerates the derived sections of `docs/evals.md` from the rubrics and real runs of BOTH harnesses, so the two families of bar and the difference between them cannot drift apart on the page. `--check` runs in `make eval`. |
...
| `load_demo_book.py` | Loads the shipped fictional obligor book (`src/credit_portfolio_ews/data/demo_book/`) into the managed BigQuery dataset, so the deployment reads the same records the laptop reads from DuckDB rather than an empty warehouse. It fills tables and never creates them, so the Terraform must be applied first; it refuses to overwrite a dataset whose own `book_manifest` does not declare what it holds fictional; and `--tenant` is required, because rows stamped with anything but the tenant the identity adapter resolves are invisible to every real user and an invisible row reads exactly like an empty dataset. It loads into the schema each table already has rather than letting a truncating load autodetect one and replace it: an autodetected schema relaxes every mode to NULLABLE and reorders the columns, which makes the next `terraform plan` report the table as must-be-replaced and destroy the rows it holds. `--dry-run DIR` writes the NDJSON it would load and stops, needing no credentials and no `[gcp]` extra. |
...
| `rename_fork.py` | The one-pass rebrand for a fork: the package name (which is also the console script), the `CREDITEWS_` env prefix including the bare token `infra/terraform/render.tf.json` carries and the backticked form the docs carry, the Terraform `name_prefix` resource stem (`doc7-svc`) and the distribution id. Prints a plan and writes nothing without `--yes`; skips itself, so the renamer is not left half-rewritten. It touches no policy number, so a renamed fork is a rebranded scorecard and not a calibrated one. See `docs/ADOPTING.md`. |
