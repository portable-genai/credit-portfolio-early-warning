...
* the grade registry declares NO write method in any profile, which is the enforcement of
  "never re-grades an obligor autonomously";
...
from credit_portfolio_ews import (
    config,
)
from credit_portfolio_ews.domain import (
    kernel,
)
from credit_portfolio_ews.ports import (
    PORT_PROTOCOLS,
)
...
def check_the_grade_registry_cannot_write() -> str:
    """The vertical's headline control, checked by INTROSPECTION rather than by assertion.

    Two halves, because a Protocol is structural: the Protocol must declare exactly the two read
    methods, and every BOUND adapter in every profile must carry no public attribute that looks
    like a write. Both were shown red against a planted ``set_grade``.
    """
    declared = demo.protocol_methods(GradeRegistryPort)
    if declared != ["list_obligors", "obligor"]:
        raise AssertionError(
            "the registry Protocol declares more than the two reads: " + str(declared)
        )
    for profile in config.KNOWN_PROFILES:
        adapter = config.build_container(_settings(profile)).grade_registry
        found = demo.registry_write_methods(adapter)
        if found:
            raise AssertionError(profile + "/grade_registry exposes a write verb: " + str(found))
    return "read-only Protocol, and no write verb on any bound adapter in any profile"
...
CHECKS: tuple[Check, ...] = (
    ...
    Check("grade registry cannot write", check_the_grade_registry_cannot_write),
