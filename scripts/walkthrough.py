...
def _check_opened(state: dict[str, Any]) -> list[str]:
    ...
    # The two vertical facts on this page, checked rather than narrated.
    ladder = facts.get("band_ladder") or []
    if not ladder:
        problems.append("the band ladder is empty, so every obligor would land in one grade")
    if not facts.get("ladder_monotonic"):
        problems.append("the band ladder is not monotonic in both score and grade rank")
    caps = facts.get("family_caps") or {}
    if not caps:
        problems.append("no family caps were read from the policy block")
    if facts.get("grade_registry_write_methods"):
        problems.append(
            "the grade registry exposes a write-verb method: "
            + str(facts["grade_registry_write_methods"])
        )
    floor = int(facts.get("first_adverse_band", 0))
    for family in ("external", "process"):
        if int(caps.get(family, 0)) >= floor:
            problems.append(
                family
                + " may reach the first adverse band alone (cap "
                + str(caps.get(family))
                + " against floor "
                + str(floor)
                + ")"
...
def _check_routine(state: dict[str, Any]) -> list[str]:
    facts = _facts(state)
    problems: list[str] = []
    if int(facts.get("days_past_due", 0)) != 41:
        problems.append("the routine obligor is not the forty-one-day arrears case")
    if int(facts.get("effective_days_past_due", -1)) != 0:
        problems.append("the materiality gate did not stop the past-due clock")
    if facts.get("arrears_material"):
        problems.append("an immaterial arrear was treated as material")
    signals = {rule: weight for rule, weight in facts.get("signals", [])}
    if "ews-arrears-immaterial" not in signals:
        problems.append("the rule that did not fire was not recorded")
    elif signals["ews-arrears-immaterial"] != 0:
        problems.append("the non-firing record carried a non-zero weight")
    if int(facts.get("composite_score", -1)) != 0:
        problems.append("a clean obligor scored above zero")
    if facts.get("movement") != "affirm":
        problems.append("a clean obligor did not affirm")
    if facts.get("requires_human_review"):
        problems.append("a clean obligor escalated")
    if facts.get("review_ref"):
        problems.append("a review was manufactured for a clean obligor")
    if state["totals"]["escalated"] != 0:
        problems.append("escalation count moved on a clean obligor")
    return problems
...
def _check_escalation(state: dict[str, Any]) -> list[str]:
    facts = _facts(state)
    problems: list[str] = []
    families = facts.get("family_scores") or {}
    financial = families.get("financial") or [0, 0]
    if financial != [73, 40]:
        problems.append("the financial family did not sum 73 and contribute 40: " + str(financial))
    external = families.get("external") or [0, 0]
    if external[1] != 0:
        problems.append("an unconfirmed media item contributed to the score")
    if int(facts.get("composite_score", -1)) != 65:
        problems.append("the composite is not 65: " + str(facts.get("composite_score")))
    if facts.get("band_grade") != "special_mention":
        problems.append("the band alone did not say special mention")
    if facts.get("proposed_grade") != "substandard":
        problems.append("the arrears floor did not classify this obligor substandard")
    if "floor-arrears-default" not in facts.get("applied_floors", []):
        problems.append("the ninety-day arrears floor is not recorded as applied")
    if int(facts.get("notches", 0)) != 2:
        problems.append("a two-notch downgrade was not proposed")
    if not facts.get("requires_human_review"):
        problems.append("a two-notch downgrade proposal did not escalate")
    if not facts.get("review_ref"):
        problems.append("rule R8: an escalation was flagged but not routed")
    if len(facts.get("confirmation_requested", [])) != 1:
        problems.append("the unconfirmed item was not surfaced for confirmation")
    if facts.get("grade_applied") is not False:
        problems.append("something claimed a grade was applied")
    totals = state["totals"]
...
def _check_redaction(state: dict[str, Any]) -> list[str]:
    facts = _facts(state)
    problems: list[str] = []
    if not facts.get("planted_identifier_in_raw"):
        problems.append(
            "the planted identifier is absent from the raw assessment, so the "
            "redaction check asserts nothing"
        )
    if facts.get("planted_identifier_leaked"):
        problems.append("the planted identifier reached the immutable audit record")
    if int(facts.get("model_prompts_sent", 0)) < 1:
        problems.append(
            "the service sent the model nothing, so the prompt checks below observed no call"
        )
    if facts.get("planted_identifier_in_prompt"):
        problems.append("the planted identifier reached a prompt the service sent")
    if not facts.get("prompt_carries_the_masked_clause"):
        problems.append(
            "the clause text never reached the prompt at all, so the masking assertion above "
            "would pass for the wrong reason"
        )
    if facts.get("planted_identifier_in_span"):
        problems.append("the planted identifier reached a span attribute")
    if facts.get("applied_floors"):
        problems.append("a live waiver did not suppress the covenant-breach floor")
    if int(facts.get("waived_weight", 0)) <= 0:
        problems.append("the waiver removed the SIGNAL as well as the floor")
    if not facts.get("review_ref"):
        problems.append("rule R8: the expiring-waiver proposal was not routed")
    return problems
...
def _check_review_queue(state: dict[str, Any]) -> list[str]:
    ...
    if int(facts.get("pending", 0)) != int(state["totals"]["routed"]):
        problems.append("the queue does not hold every routed proposal")
    if facts.get("grade_applied") is not False:
        problems.append("a payload claimed a grade had been applied")
    if int(facts.get("registry_writes", 1)) != 0:
        problems.append("the grade registry exposes a write path")
    if 2 not in facts.get("required_approvals", []):
        problems.append("the non-performing proposal did not demand dual control")
    return problems
...
def _check_audit(state: dict[str, Any]) -> list[str]:
    ...
    if int(facts.get("entries", 0)) < 3:
        problems.append("fewer audit records than obligors reviewed")
    if not facts.get("record_names_grade"):
        problems.append("no record names the proposed grade")
    if not facts.get("record_names_floor"):
        problems.append("no record names the floor rule that lifted the proposal")
    return problems
