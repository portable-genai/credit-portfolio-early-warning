...
The catalog id (``Doc7``) is left alone unless ``--catalog-id`` is given: it names the entry in
...
watch-grade ladder, the covenant catalogue, region, IdP, eval golden set) that this script does
NOT do for you. In particular it does not touch `WatchPolicy`: a fork that renames the package
and ships the reference weights has rebranded a scorecard it has not calibrated.
"""
...
_OLD_PACKAGE = "credit_portfolio_ews"
_OLD_ENV_PREFIX = "CREDITEWS_"
_OLD_BARE_PREFIX = "CREDITEWS"
_OLD_RESOURCE = "doc7-svc"
_OLD_DIST = "credit-portfolio-early-warning"
_OLD_CATALOG_ID = "Doc7"
...
def _rewrite_text(text: str, args: argparse.Namespace) -> tuple[str, int]:
    ...
    # The env prefix, only where it PREFIXES an upper token (CREDITEWS_PROFILE), never mid-word.
    ...
    # this repo as "env `CREDITEWS`"). Without this, a fork run with --include-docs still tells
...
def main() -> int:
    ...
    ap.add_argument(
        "--resource",
        required=True,
        help="new cloud resource stem (Terraform name_prefix), e.g. acme-ews",
    )
    ...
    print("Then finish the human steps in docs/ADOPTING.md (watch policy, covenants, region, IdP).")
    return 0
